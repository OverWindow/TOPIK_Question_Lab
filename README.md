...
3. `50문항 세트`에서 1~50번 누락·중복과 제외 사유를 확인하고 복수 후보가 있으면 사용할 문항을 선택합니다.
...
내용을 다시 발행하면 기존 버전을 재사용하고, 문항 내용이나 메타데이터가 바뀌면
`item_version`과 해당 모델·영역의 `set_version`이 증가합니다. 듣기 그림·그래프는
